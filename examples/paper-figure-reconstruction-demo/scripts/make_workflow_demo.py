...
def title_card() -> Image.Image:
    ...
    draw.text((100, 549), "v1.3.0 · MIT · real outputs and audit evidence", font=font(24, True), fill=(244, 184, 80))
    return image
...
def capability_card() -> Image.Image:
    image, draw = canvas()
    header(draw, "Current capability", "What v1.3.0 can actually deliver", "One plugin, three focused skills, explicit editor and evidence boundaries.")
    rows = [
...
def end_card() -> Image.Image:
    ...
    draw.text((72, 594), "MIT License · v1.3.0", font=font(22, True), fill=WHITE)
    return image
